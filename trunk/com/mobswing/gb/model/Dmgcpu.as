package com.mobswing.gb.model
{
	...
	public class Dmgcpu
	{
		 ...
		 public	 var graphicsChip:GraphicsChip;
		 public  var soundChip:SoundChip;
		 public  var gameLink:GameLink;
		  ...
		  */
		public function Dmgcpu(c:Cartridge, l:GameLink, a:DisplayObject)
		{
		  cartridge = c;
		  gameLink = l;
//		  if (gameLink != null) gameLink.setDmgcpu(this);
		...
		/*  if ((addr < 0) || (addr > 65535)) {
		 ...
		 //    System.out.println("Tried to write to ROM! PC = " + JavaBoy.hexWord(pc) + ", Data = " + JavaBoy.hexByte(JavaBoy.unsign((byte) data)));
		   ...
		   case 0x8000 :
		   case 0x9000 :
		    graphicsChip.addressWrite(addr - 0x8000, data);
		    break;
		   ...
		   case 0xF000 :
		    if (addr < 0xFE00) {
		     try {
		      ...
		      trace("Address error: " + addr + " pc = " + Javaboy.hexWord(pc));
		     }
		    } else if (addr < 0xFF00) {
		...
		//   registers[addr - 0xFF00] = (byte) data;
		   ...
		   trace("Attempt to write to address "+ Javaboy.hexWord(addr));
		  }
		 }
		...
		//  System.out.println("Triggered:" + intr);
		 ...
		 public function initiateInterrupts():void {
		   if (timaEnabled && ((instrCount % instrsPerTima) == 0)) {
		    if (Javaboy.unsign(ioHandler.registers.read(05)) == 0) {
		   ...
		   if ((instrCount % INSTRS_PER_HBLANK) == 0) {
			...
			var cline:int = Javaboy.unsign(ioHandler.registers.read(0x44)) + 1;
			if (cline == 152) cline = 0;
		
		    if (((ioHandler.registers.read(0xFF) & INT_LCDC) != 0) &&
			     ((ioHandler.registers.read(0x41) & 64) != 0) &&
		       (Javaboy.unsign(ioHandler.registers.read(0x45)) == cline) && ((ioHandler.registers.read(0x40) & 0x80) != 0) && (cline < 0x90)) {
		...
		//	 System.out.println("** LCDC Int **");
		    ...
		    if (Javaboy.unsign(ioHandler.registers.read(0x44)) == 143) {
		...
		//	   System.out.println("VBlank LCDC!");
		     ...
		     if ((Parameters.viewSpeedThrottle) && (graphicsChip.frameWaitTime >= 0)) {
		//      System.out.println("Waiting for " + graphicsChip.frameWaitTime + "ms.");
		      try {
		       Thread.sleep(graphicsChip.frameWaitTime);
		      } catch (e:Error) {
		       // Nothing.
		      }
		     }
			...
			graphicsChip.notifyScanline(Javaboy.unsign(ioHandler.registers.read(0x44)));
		    ioHandler.registers.write(0x44, (Javaboy.unsign(ioHandler.registers.read(0x44)) + 1));
		//	System.out.println("Reg 44 = " + JavaBoy.unsign(ioHandler.registers[0x44]));
		
		    if (Javaboy.unsign(ioHandler.registers.read(0x44)) >= 153) {
		//     System.out.println("VBlank");
			 ...
			 if (Javaboy.runningAsApplet) {
		      (applet as Javaboy).drawNextFrame();
			 }
		     try {
		...
		public	function beforeExe():void
		{
			terminate = false;
			running = true;
			graphicsChip.startTime = Parameters.getCurrentTime;
		}
		...
		//        terminate = true;
		    ...
		    default :
		        ...
		        if ((b1 & 0xC0) == 0x80) {       // Byte 0x10?????? indicates ALU op
		         ...
		         trace("Unrecognized opcode (" + Javaboy.hexByte(b1) + ")");
		         terminate = true;
		         pc++;
		         break;
		        }
		...
		/*   if ((hl & 0xFFFF0000) != 0) {
		 ...
		 public function disassemble(address:int, numInstr:int):String {
		  ...
		  for (var r:int = 0; r < numInstr; r++) {
		   var b1:int = Javaboy.unsign(addressRead(address));	//short
		   var offset:int = addressRead(address + 1);	//short
		   var b3:int = Javaboy.unsign(addressRead(address + 2));	//short
		   var b2:int = Javaboy.unsign(offset);	//short
		
		   var instr:String = "Unknown Opcode! (" + Javaboy.unsign(b1).toString(16) + ")";
		...
		//    System.out.println("LD Op src" + sourceRegister + " dest " + destRegister);
		 ...
		 //     1 0 1 1 1 R R R    Compare?        CP
		   ...
		   if ((Javaboy.unsign(b1) >= 0x80) && (Javaboy.unsign(b1) <= 0xBF)) {
		    sourceRegister = Javaboy.unsign(b1) & 0x07;
		    operation = (Javaboy.unsign(b1) & 0x38) >> 3;
		 ...
		 //     1 1 N N N R R R    Set Bit n             SET
		   ...
		   trace(Javaboy.hexWord(address) + ": " + Javaboy.hexByte(Javaboy.unsign(b1)));
		
		   if (instrLength >= 2) {
		    trace(" " + Javaboy.hexByte(Javaboy.unsign(b2)));
		   } else {
		    trace("   ");
		   }
		
		   if (instrLength == 3) {
		    trace(" " + Javaboy.hexByte(Javaboy.unsign(b3))+ "  ");
		   } else {
		    trace("     ");
		   }
