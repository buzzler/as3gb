package com.mobswing.gb.control
{
	...
	public class Cartridge
	{
		...
		//  if (disposed) trace("oh.  dodgy cartridge");
		...
		//  } else {
		   if ((addr >= 0xA000) && (addr <= 0xBFFF)) {
		    switch (cartType) {
		     ...
		     case 0x13 : {	/* MBC3 */
			  if (ramBank >= 0x04) {
		//	   trace("Reading RTC reg " + ramBank + " is " + RTCReg[ramBank - 0x08]);
		...
		//  }
		 ...
		 public function getMapInfo():String {
		  ...
		  switch (cartType) {
		   ...
		   case 1 /* MBC1      */ :
		     return "MBC1: ROM bank " + Gameboy.hexByte(currentBank) + " mapped to " +
		            " 4000 - 7FFFF";
		   case 2 /* MBC1+RAM  */ :
		   case 3 /* MBC1+RAM+BATTERY */ :
		     out = "MBC1: ROM bank " + Gameboy.hexByte(currentBank) + " mapped to " +
		           " 4000 - 7FFFF.  ";
		     if (mbc1LargeRamMode) {
		   ...
		   case 5 :
		   case 6 : 
		    return "MBC2: ROM bank " + Gameboy.hexByte(currentBank) + " mapped to 4000 - 7FFF";
		
		   case 0x19 :
		   case 0x1C :
		    return "MBC5: ROM bank " + Gameboy.hexByte(currentBank) + " mapped to 4000 - 7FFF";
		 ...
		 public function restoreMapping():void {
		  if (savedBank != -1) {
		   trace("- ROM Mapping restored to bank " + Gameboy.hexByte(savedBank));
		...
		//      trace("RAM bank " + ramBank + " selected!");
		     } else {
		...
		# *  Files are compatible with VGB-DOS.
		 ...
		 public function verifyChecksum():Boolean {
		  var checkSum:int = (Gameboy.unsign(rom.read(0x14E)) << 8) + Gameboy.unsign(rom.read(0x14F));
		
		  var total:int = 0;                   // Calculate ROM checksum
		  for (var r:int=0; r < rom.length; r++) {
		   if ((r != 0x14E) && (r != 0x14F)) {
		    total = (total + Gameboy.unsign(rom.read(r))) & 0x0000FFFF;
		   }
		  }
